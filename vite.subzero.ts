...
function psql() {
    return function (file: string, dbUri: string, rootDir: string) {
        ...
        const result = spawnSync('psql', [dbUri, '-f', `${normalizedFile}`, '--quiet'], { stdio: 'inherit', cwd: rootDir });
        if (result.status !== 0) {
...
function pg() {
    ...
    return async function (file: string, dbUri: string, rootDir: string) {
        if (!client || !client._connected || client._ending) {
...
export default function subzeroPlugin(projectRootDir, config?: PluginConfig): PluginOption {
    ...
    const runSql = result.status === 0 ? psql() : pg();
